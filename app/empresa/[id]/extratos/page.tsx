...
import { Empresa, BancoEmpresa, SolicitacaoExtrato } from '@/lib/types';
...
export default function ExtratosEmpresa() {
  ...
  const handleCobrarTodosPendentes = async () => {
    for (const b of bancos) {
      ...
      if (s !== 'recebido' && s !== 'importado') {
        await handleRegistrarSolicitacao(b.id);
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                <Link href={`/empresa/${empresaId}`} className="hover:underline">
                  Ver checklist da empresa →
                </Link>
              </p>
            </div>
            <button
              ...
              return (
                ...
                >
                  <span className="flex items-center justify-center gap-1.5">
                    {m.label}
                    {pontoCor && <span className={`inline-block h-1.5 w-1.5 rounded-full ${pontoCor}`} />}
                  </span>
                </button>
              );
            })}
        ...
        {/* Resumo do mês */}
        <div className={`mb-6 border rounded-md px-5 py-4 ${resumoClasse}`}>
          ...
          <header className="px-5 py-4 border-b border-slate-200 flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                Cadastre bancos, registre cobranças e atualize status individual.
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <form onSubmit={handleAddBanco} className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-xs font-medium text-slate-700 border border-slate-300 hover:bg-slate-50 disabled:opacity-50 px-3 py-1.5 rounded-md transition"
                >
                  {adicionandoBanco ? 'Adicionando...' : '+ Banco'}
                </button>
              </form>
            </div>
          </header>
